...
export default function UserDashboard() {
    ...
    const fetchStats = async () => {
        const [projects, directory] = await Promise.all([
            supabase.from('projects').select('*', { count: 'exact', head: true }).eq('status', 'activo'),
            supabase.from('profiles').select('*', { count: 'exact', head: true }).in('role', ['provider', 'client'])
        ]);

        setStats({
            projectsCount: projects.count || 0,
            directoryCount: directory.count || 0
        });
    };
    ...
    return (
        <div className="admin-layout" style={{ flexDirection: isMobile ? 'column' : 'row' }}>
            ...
            <main className="admin-main">
                <header className="admin-header">
                    <div>
                        <h1 style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                            {activeTab === 'dashboard' && 'Bienvenido'}
                            {activeTab === 'projects' && 'Proyectos Vigentes'}
                            {activeTab === 'directory' && 'Directorio de Aliados'}
                        </h1>
                        <p style={{ color: '#64748b', fontSize: '0.95rem' }}>
                            Hola {profile?.full_name?.split(' ')[0] || 'Usuario'}, explora el ecosistema IGESCOL.
                        </p>
                    </div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 20 }}>
                        <div style={{ display: isMobile ? 'none' : 'flex', alignItems: 'center', gap: 12, background: 'white', padding: '6px 15px', borderRadius: 30, boxShadow: '0 2px 10px rgba(0,0,0,0.05)' }}>
                            ...
                            <span style={{ fontWeight: 600, fontSize: '0.9rem', color: '#0A2540' }}>{profile?.full_name || 'Usuario'}</span>
                        </div>
                    </div>
                </header>
                    ...
                    <div className="dashboard-view">
                        <div className="stats-grid">
                            ...
                            <div className="stat-card">
                                ...
                                <div className="stat-info">
                                    <h3>Aliados</h3>
                                    <div className="value">{stats.directoryCount}</div>
                                </div>
                            </div>
                        </div>

                        <div className="admin-card">
                            <h2 style={{ fontSize: '1.2rem', color: '#0A2540', marginBottom: 20 }}>Acciones de Usuario</h2>
                            <p style={{ color: '#64748b', marginBottom: 20 }}>Como usuario registrado, puedes visualizar los proyectos en ejecución y conocer a nuestros aliados estratégicos.</p>
                            <div style={{ display: 'flex', gap: 15 }}>
                                <button onClick={() => setActiveTab('projects')} style={{ padding: '12px 20px', background: '#0089A7', color: 'white', border: 'none', borderRadius: 10, fontWeight: 700, cursor: 'pointer' }}>VER PROYECTOS</button>
                                <button onClick={() => setActiveTab('directory')} style={{ padding: '12px 20px', background: 'white', color: '#0089A7', border: '1px solid #0089A7', borderRadius: 10, fontWeight: 700, cursor: 'pointer' }}>VER DIRECTORIO</button>
                            </div>
                        </div>
                    </div>
                )}
                ...
                {activeTab === 'directory' && <AdminUsers initialRole="directory_providers_clients" readOnly={true} />}
            </main>
        </div>
    );
}
